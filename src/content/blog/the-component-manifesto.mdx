...
// ✅ Let children pass through when forwarding to a single element
export function Badge({ variant, ...htmlProps }: BadgeProps) {
  return <span className={getStyles(variant)} {...htmlProps} />;
}
...
export function Button({ variant, size, ...htmlProps }: ButtonProps) {
  ...
  // Note: children automatically passes through via htmlProps spread
}

// ✅ Only destructure children when you're actually manipulating it
export function Card({ variant, children }: CardProps) {
  return (
    <div className={getStyles(variant)}>
      <div className="card-content">
        {children} {/* We're wrapping children in our own structure */}
      </div>
    </div>
  );
}

// ❌ Unnecessarily destructuring children that just gets passed through
export function BadButton({ variant, children, ...htmlProps }: BadButtonProps) {
  return <button className={getStyles(variant)} {...htmlProps}>
    {children} {/* Redundant - children would pass through htmlProps */}
  </button>;
}
```

**The Children Rule**: Only destructure `children` when you're manipulating, validating, or wrapping it in your own element structure. If you're forwarding props to a single underlying element, let `children` pass through via the spread operator.
